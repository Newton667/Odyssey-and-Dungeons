...
- **AC Calculation** — Auto-calculates from equipped armor + DEX + shield; with no armor, Barbarian/Monk Unarmored Defense via `unarmoredBaseAC`; homebrew armor adds its `bonus` (built-in magic armor already includes it). The value is written back to `char.armorClass` only once equipment data has loaded
...
- **Level Up** — a header **Level Up** button (milestone characters; XP characters use the XP bar) opens the modal. HP gain = die/average + effective CON (min 1) + Tough; current HP rises by the gain; one hit die is added. Cancel and Confirm are disabled while the HP die rolls. Adding a second class migrates the first class's Progression picks (`migrateSingleClassChoices`)
