...
Characters may hold a `classes` array (`[{class, subclass, level}]`) alongside the summary fields `class`/`subclass` (primary = `classes[0]`) and `level` (total). Single-class characters omit `classes` — and a stored one-element array (written by older sheet level-ups) is ignored in favour of the top-level fields. All class-derived math (spell slots, features, extra attack, hit dice, proficiency bonus) flows through `utils/multiclass.js`, which normalizes both shapes via `getCharClasses(char)` so existing single-class saves need no migration. See `known-patterns-and-gotchas.md` → "Multiclassing."
