...
### Git Safe Directory
Windows users cloning via `start.bat` may hit "dubious ownership" errors. The auto-update endpoint adds `safe.directory` config before any git operations. Both launchers do the same (`start.bat` and `start.sh`) — keep it before any other `git` call; a FUSE-mounted NTFS partition on Linux is a classic trigger too.

### `node_modules` is platform-specific — never share it across OSes without rebuilding
Vite's rollup and esbuild install a native binary for the current OS as optional dependencies; a tree built on Windows fails on Linux with `Cannot find module @rollup/rollup-linux-x64-gnu` (and vice versa). This checkout lives on a shared NTFS partition and is used from both. `scripts/ensure-deps.js` owns the fix: it writes a marker `node_modules/.ond-platform` (`linux-x64`, `win32-x64`, …) and wipes + reinstalls the tree when the marker is missing or does not match the running `process.platform-process.arch`. Both launchers (`start.bat`, `start.sh`) and the root `postinstall` call it; **do not add a bare `npm install` to a launcher** — it does not repair a foreign tree. `node --test scripts/ensure-deps.test.js` covers the decision logic (it deliberately runs on Node's built-in runner, because vitest itself cannot start on a foreign tree).

### Batch: `%VAR%` inside a parenthesised block is expanded when the block is parsed
`if %ERRORLEVEL% neq 0` after a command inside `( … )` tests the value from *before* the block was entered, not the command's result. Use `!ERRORLEVEL!` (`setlocal enabledelayedexpansion` is on in `start.bat`) or restructure with `goto` labels so the check is on its own line. This is what made `start.bat`'s "Download failed" branch unreachable — `git clone` could fail and the script carried on as if it had succeeded.

### Batch: `%~dp0` never changes
It is the script file's own directory, not the current directory; `cd` + `goto START` cannot relaunch "from another folder" — the next `cd /d "%~dp0"` goes straight back, and that was the infinite "Downloading… / Download complete!" loop. To hand off to a copy elsewhere, `start "" /D "<dir>" "<dir>\start.bat"` and then either `exit /b` or, as `start.bat` does on purpose, fall into a `timeout`-paced message loop. **The `:DELETE_ME` loop in `start.bat` is intentional** — it repeats "you can delete this start.bat now" every 3 s so the outer window cannot be mistaken for a stalled download. Do not "fix" it into a single message, and the `start "OND Launcher" …` line must stay outside (before) the loop so the inner launcher is spawned exactly once. In bash the equivalent hand-off is `exec`, which cannot loop afterwards because it replaces the process — that is why `start.sh` shows the message once.

### Batch: cmd.exe reads a running `.bat` by byte offset — never keep running a file you just rewrote
cmd.exe does not load a batch file into memory; after each parsed statement (or parenthesised block) it reopens the file by path and seeks to the next byte offset. So when the update block in `start.bat` runs `git reset --hard origin/main`, the file under cmd's feet is replaced, and execution resumes at the old offset inside the **new** file — 12 bytes into an `echo` line this time (one harmless "'Checking' is not recognized"), but a future layout change above that block could just as easily land inside a `start "OND Server" cmd /k …` fragment. The rule: after any in-place update of the running `.bat`, `start "" /D "%~dp0" "%~f0"` and `exit` immediately so the fresh file runs from the top. bash is **not** exposed — it keeps the old inode open and finishes the old contents — so `start.sh` needs no equivalent.
